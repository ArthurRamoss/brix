// /api/admin/mint-brz
//
// Devnet test-BRZ faucet for Colosseum judges. Mints test BRZ from the
// test mint to a recipient's associated token account so judges can run
// the demo end-to-end without any prior setup.
//
...
// the configured RPC isn't pointed at devnet. The mint is a TEST mint
// with no value, so the worst case of accidental exposure is "user has
// free play money on devnet" — fine.
//
// In real production BRZ is minted by Transfero (the issuer). We don't
// have authority. This whole file gets deleted before any non-devnet
// deploy.
//
// Admin keypair: read from BRIX_ADMIN_KEYPAIR_JSON (raw JSON array, the
// recommended path for hosted deploys) OR a filesystem path from
// BRIX_ADMIN_KEYPAIR / ANCHOR_WALLET / ~/.config/solana/id.json (local
// dev).
...
function loadAdminKeypair(): Keypair {
  // 1. Hosted-deploy path: full secret-key JSON array stored in an env var.
  const inline = process.env.BRIX_ADMIN_KEYPAIR_JSON;
  if (inline) {
    const parsed = JSON.parse(inline) as number[];
    if (!Array.isArray(parsed)) {
      throw new Error("BRIX_ADMIN_KEYPAIR_JSON must be a JSON number[] array");
    }
    return Keypair.fromSecretKey(Uint8Array.from(parsed));
  }
  // 2. Local-dev path: file at BRIX_ADMIN_KEYPAIR / ANCHOR_WALLET / default.
  const inputPath =
...
export async function POST(req: Request) {
  // Hard gate: refuse on non-devnet RPCs. The mint authority lives on the
  // server, so even a leaked endpoint can't drain mainnet BRZ — but we
  // still refuse to point at anything other than devnet as defense in
  // depth.
  ...
  try {
    ...
    return serverError(
      err instanceof Error
        ? `${err.message} (set BRIX_ADMIN_KEYPAIR_JSON or BRIX_ADMIN_KEYPAIR)`
        : err,
    );
  }
  ...
  // Build the tx: create the recipient ATA if missing, then mint. Using
  // the idempotent ATA helper means we don't need to query first.
